...
async function finalize(options) {
  ...
  for (const scenario of PLAN.requiredEvidence) {
    ...
    if (scenario === "deep-task") {
      if (!Array.isArray(evidence.layoutCases) ||
        JSON.stringify(evidence.layoutCases.map((item) => item.layout)) !== JSON.stringify(PLAN.agentLayouts)) {
        throw new Error("deep-task must contain all compact agent-layout cases in plan order");
      }
      for (const item of evidence.layoutCases) {
        const screenshot = path.join(options.evidenceDir, path.basename(String(item.screenshot || "")));
        const screenshotStat = await fs.stat(screenshot).catch(() => null);
        if (item.result?.pass !== true || item.result?.documentOverflow?.x !== false ||
          item.result?.documentOverflow?.y !== false || !screenshotStat?.isFile() || screenshotStat.size < 1) {
          throw new Error(`deep-task/${item.layout}/compact did not pass or lacks a sanitized screenshot`);
        }
        layoutCases.push(item);
      }
    }
  }
  ...
  const report = [
    ...
    `- ${layoutCases.length}/${layoutCases.length} compact template cases: PASS`,
    "- restore/reapply: PASS",
    "- screenshots: sanitized local evidence",
    "",
    "## Scenarios",
    "",
    ...PLAN.requiredEvidence.map((scenario) => `- ${scenario}: 100%, 125%, 150%, mid-height, compact-height`),
    "",
  ].join("\n");
  ...
  return {
    ...
    layoutCaseCount: layoutCases.length,
    lifecyclePass: true,
