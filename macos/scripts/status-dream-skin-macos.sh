...
# Codex process: inspect command lines instead of pgrep. On current macOS,
# pgrep deliberately omits the calling process's ancestors; Codex is an
# ancestor of commands launched from its integrated terminal, so pgrep can
# report a false negative even while the app is plainly running.
...
if [ -f "$THEME_DIR/theme.json" ]; then
  ...
  THEME_MODE="$(read_json_field "$THEME_DIR/theme.json" mode)"
  PROFILE_NICKNAME="$(read_json_field "$THEME_DIR/theme.json" nickname)"
  PROFILE_STATUS="$(read_json_field "$THEME_DIR/theme.json" status)"
  PROFILE_LEVEL="$(read_json_field "$THEME_DIR/theme.json" level)"
fi
...
if [ "$JSON" = "true" ]; then
  ...
  printf '{"session":"%s","port":%s,"injectorAlive":%s,"cdpOk":%s,"codexRunning":%s,"themeName":"%s","themeMode":"%s","profileNickname":"%s","profileStatus":"%s","profileLevel":"%s"}\n' \
    "$(json_escape "$SESSION")" "$port_json" "$(bool "$INJECTOR_ALIVE")" \
    "$(bool "$CDP_OK")" "$(bool "$CODEX_RUNNING")" "$(json_escape "$THEME_NAME")" \
    "$(json_escape "$THEME_MODE")" "$(json_escape "$PROFILE_NICKNAME")" \
    "$(json_escape "$PROFILE_STATUS")" "$(json_escape "$PROFILE_LEVEL")"
  exit 0
fi
...
printf 'theme=%s\n' "${THEME_NAME:-}"
printf 'mode=%s\n' "${THEME_MODE:-}"
printf 'profile=%s · %s · %s\n' "${PROFILE_NICKNAME:-}" "${PROFILE_STATUS:-}" "${PROFILE_LEVEL:-}"
