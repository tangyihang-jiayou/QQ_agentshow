...
QQ_SHOW_IMAGE="$("$NODE" -e 'try{const t=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"));process.stdout.write(t.decorations?.qqShow||"")}catch{}' "$THEME_DIR/theme.json")"
ASSISTANT_IMAGE="$("$NODE" -e 'try{const t=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"));process.stdout.write(t.decorations?.assistant||"")}catch{}' "$THEME_DIR/theme.json")"
for entry in "$THEME_DIR/"*; do
  [ -f "$entry" ] || continue
  base="$(/usr/bin/basename "$entry")"
  if [ "$base" != "theme.json" ] && [ "$base" != "$THEME_IMAGE" ] \
    && [ "$base" != "$QQ_SHOW_IMAGE" ] && [ "$base" != "$ASSISTANT_IMAGE" ]; then
    /bin/rm -f "$entry"
  fi
done
/bin/rm -rf "$stage"
