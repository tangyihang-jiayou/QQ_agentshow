...
if (!["validate", "install", "restore"].includes(mode) || !configPath || !backupPath) {
  throw new Error("Usage: theme-config.mjs <validate|install|restore> <config-path> <backup-path>");
}
...
async function main() {
  ...
  if (mode === "validate") {
    try {
      ...
      const backup = JSON.parse(decodeStrictUtf8(backupBytes, "Theme backup"));
      validateBackup(backup);
      console.log("Existing selective theme backup is valid.");
    } catch (error) {
      if (error.code === "ENOENT") {
        console.log("No existing selective theme backup; a valid backup will be created during installation.");
        return;
      }
      throw new Error(`Existing theme backup is unsafe: ${error.message}`);
    }
    return;
  }
