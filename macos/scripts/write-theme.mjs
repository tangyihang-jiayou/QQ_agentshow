...
if (mode === "reset-demo") {
  ...
  if (!home) throw new Error("HOME is required to locate the QQ_agentshow theme directory.");
  const expectedOutput = path.resolve(
    home,
    "Library/Application Support/CodexDreamSkinStudio/theme",
  );
  if (outputDir !== expectedOutput) {
    throw new Error(`reset-demo only accepts the exact managed theme directory: ${expectedOutput}`);
  }
  ...
  if (outputStat?.isSymbolicLink() || (outputStat && !outputStat.isDirectory())) {
    throw new Error("Managed theme path must be a real directory, not a symbolic link or file.");
  }
  const realOutput = await fs.realpath(outputDir).catch((error) => {
    if (error.code === "ENOENT") return null;
    throw error;
  });
  const realStateRoot = await fs.realpath(path.dirname(outputDir));
  if (realOutput && realOutput !== path.join(realStateRoot, "theme")) {
    throw new Error("Managed theme path resolves outside its verified state directory.");
  }
