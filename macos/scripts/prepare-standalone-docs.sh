...
if [ "$#" -eq 2 ]; then
  ...
  if [ -z "$DOCS_SOURCE" ] && [ -n "$COMPACT_GUIDE" ]; then
    ...
    INSTALLATION_SOURCE="$SCRIPT_ROOT/docs/INSTALLATION.md"
    INSTALLATION_TARGET="$ARCHIVE_ROOT/docs/INSTALLATION.md"
    NOTICE_TARGET="$ARCHIVE_ROOT/NOTICE.md"
    [ -f "$INSTALLATION_SOURCE" ] || {
      printf 'Standalone installation guide is missing: %s\n' "$INSTALLATION_SOURCE" >&2
      exit 1
    }
    ...
    if [ "$INSTALLATION_SOURCE" != "$INSTALLATION_TARGET" ]; then
      /bin/cp "$INSTALLATION_SOURCE" "$INSTALLATION_TARGET"
    fi
    ...
    if /usr/bin/grep -E -q '(^cd macos$|`macos/(LICENSE|presets/|assets/|scripts/)|\.\./macos/|\./install\.sh|\./restore\.sh)' \
      "$COMPACT_TARGET" "$INSTALLATION_TARGET" "$NOTICE_TARGET"; then
      printf 'Standalone QQ2007 documentation retains a repository-only macOS path.\n' >&2
      exit 1
    fi
    /usr/bin/grep -F -q './scripts/install-dream-skin-macos.sh --check' "$INSTALLATION_TARGET" || {
