...
import { readImageMetadata, readPngTransparency } from "./image-metadata.mjs";
...
function choiceOption(name, fallback, choices) {
...
try {
  ...
  const profile = {
    nickname: textOption("nickname", baseProfile.nickname || "Codex 小企鹅", 40),
    signature: textOption("signature", baseProfile.signature || "不要迷恋哥，哥只是个传说", 120),
    level: textOption("level", baseProfile.level || "255", 16),
  ...
  const baseAgentShow = records[0].raw.agentShow || {};
  const agentShow = {
    layout: choiceOption("agent-layout", baseAgentShow.layout || "classic-chat", ["classic-chat", "workbench", "minimal"]),
    petMotion: choiceOption("pet-motion", baseAgentShow.petMotion || "calm", ["off", "calm", "playful"]),
    conversationPreview: choiceOption(
      "conversation-preview",
      baseAgentShow.conversationPreview || "real",
      ["real", "masked", "off"],
    ),
    completionSound: options.has("completion-sound")
      ? options.get("completion-sound") === "on" : baseAgentShow.completionSound !== false,
  };
  if (options.has("completion-sound") && !["on", "off"].includes(options.get("completion-sound"))) {
    throw new Error("completion-sound must be on or off.");
  }
  ...
  async function validatedPng(optionName) {
    ...
    if (optionName === "assistant") {
      const transparency = readPngTransparency(bytes);
      if (!transparency?.hasAlphaChannel || !transparency.hasTransparentPixels || !transparency.hasVisiblePixels) {
        throw new Error("assistant must be a visible PNG cutout with transparent pixels.");
      }
    }
    return bytes;
  }
