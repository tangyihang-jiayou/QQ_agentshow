...
if [ "$RESET_DEMO" = "true" ]; then
  ...
  [ ! -L "$IMAGE" ] || fail "Selected image must not be a symbolic link: $IMAGE"
  ...
  [ ! -L "$prepared" ] || fail "Theme image destination must not be a symbolic link: $prepared"
  if [ -e "$prepared" ]; then [ -f "$prepared" ] || fail "Theme image destination is not a regular file: $prepared"; fi
