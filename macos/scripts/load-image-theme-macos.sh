...
ensure_managed_directory "$IMAGES_DIR"
ensure_managed_directory "$THEMES_ROOT"
ensure_managed_directory "$THEME_DIR"
...
[ ! -L "$IMAGE" ] || fail "Image source must not be a symbolic link: $IMAGE"
...
theme_id="img-$(/bin/date '+%Y%m%d%H%M%S')-$(/usr/bin/uuidgen | /usr/bin/tr -d '-' | /usr/bin/cut -c1-12)"
...
# Always use the validated Node runtime bundled with official Codex.
ensure_node_runtime

image_name="background.jpg"
temporary="$(/usr/bin/mktemp "$THEME_DIR/.background.XXXXXX")"
prepared="$THEME_DIR/$image_name"
cleanup_temporary() { /bin/rm -f "$temporary"; }
trap cleanup_temporary EXIT
[ ! -L "$prepared" ] || fail "Active theme image must not be a symbolic link: $prepared"
if [ -e "$prepared" ]; then [ -f "$prepared" ] || fail "Active theme image is not a regular file: $prepared"; fi
...
[ ! -e "$lib_dir" ] && [ ! -L "$lib_dir" ] || fail "Theme library destination already exists: $lib_dir"
/bin/mkdir "$lib_dir"
/bin/chmod 700 "$lib_dir"
/bin/cp "$THEME_DIR/$image_name" "$THEME_DIR/theme.json" "$lib_dir/"
...
if [ "$src_dir/$(/usr/bin/basename "$IMAGE")" != "$img_dir/$(/usr/bin/basename "$IMAGE")" ]; then
  [ ! -L "$dest_lib_img" ] || fail "Image library destination must not be a symbolic link: $dest_lib_img"
  if [ -e "$dest_lib_img" ]; then [ -f "$dest_lib_img" ] || fail "Image library destination is not a regular file: $dest_lib_img"; fi
  library_temporary="$(/usr/bin/mktemp "$IMAGES_DIR/.image-import.XXXXXX")"
  if /bin/cp "$IMAGE" "$library_temporary" 2>/dev/null; then
    /bin/chmod 600 "$library_temporary"
    /bin/mv -f "$library_temporary" "$dest_lib_img"
  else
    /bin/rm -f "$library_temporary"
  fi
fi
