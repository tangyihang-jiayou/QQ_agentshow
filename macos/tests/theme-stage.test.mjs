...
const injectorScript = path.join(macosRoot, "scripts", "injector.mjs");
...
function checkPayload(themeDir) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [injectorScript, "--check-payload", "--theme-dir", themeDir], {
      stdio: ["ignore", "pipe", "pipe"],
    });
    let stdout = "";
    let stderr = "";
    ...
    child.once("close", (code) => {
      ...
      else reject(new Error(stderr || `injector check exited with ${code}`));
    });
  });
}
...
try {
  ...
  const decorated = path.join(tempRoot, "themes", "preset-decorated");
  const decoratedStage = path.join(tempRoot, "decorated-stage");
  await fs.mkdir(decorated);
  await fs.mkdir(decoratedStage);
  await fs.copyFile(fixtureAsset, path.join(decorated, "background.png"));
  await fs.copyFile(fixtureAsset, path.join(decorated, "qq-show.png"));
  await fs.copyFile(fixtureAsset, path.join(decorated, "assistant.png"));
  await fs.writeFile(
    path.join(decorated, "theme.json"),
    `${JSON.stringify({
      schemaVersion: 1,
      id: "preset-decorated",
      image: "background.png",
      decorations: { qqShow: "qq-show.png", assistant: "assistant.png" },
    })}\n`,
  );
  await runStage(decorated, decoratedStage);
  assert.deepEqual(
    (await fs.readdir(decoratedStage)).sort(),
    ["assistant.png", "background.png", "qq-show.png", "theme.json"],
  );

  const undecoratedDeep = path.join(tempRoot, "themes", "undecorated-deep");
  await fs.mkdir(undecoratedDeep);
  await fs.copyFile(fixtureAsset, path.join(undecoratedDeep, "background.png"));
  await fs.writeFile(
    path.join(undecoratedDeep, "theme.json"),
    `${JSON.stringify({ schemaVersion: 1, id: "undecorated-deep", image: "background.png", mode: "deep" })}\n`,
  );
  assert.deepEqual((await checkPayload(undecoratedDeep)).decorationKeys,
    ["assistant", "qqShow", "qqShowClassicBoy", "qqShowClassicGirl"],
    "A valid deep theme without optional decorations must receive bundled defaults.");
